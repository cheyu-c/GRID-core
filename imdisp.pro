...
FUNCTION IMDISP_IMSCALE, IMAGE, RANGE=RANGE, BOTTOM=BOTTOM, NCOLORS=NCOLORS, $
  NEGATIVE=NEGATIVE, SQUARE_ROOT=SQUARE_ROOT
...
;- Apply square root enhancement if required
if keyword_set(square_root) then begin
  table = byte(round(sqrt(lindgen(ncolors + 1L))/sqrt(ncolors) * ncolors))
  scaled = table[scaled]
endif
...
PRO IMDISP, IMAGE, RANGE=RANGE, BOTTOM=BOTTOM, NCOLORS=NCOLORS, $
  ...
  AXIS=AXIS, NEGATIVE=NEGATIVE, SQUARE_ROOT=SQUARE_ROOT, _EXTRA=EXTRA_KEYWORDS
...
;
;    SQUARE_ROOT If set, a square root enhancement is applied to the scaled
;                image. This keyword is ignored if the NOSCALE keyword is set.
;
; OUTPUTS:
;    None.
;
...
if (keyword_set(noscale) eq 0) then begin
  ...
  case true of
    0 : scaled = imdisp_imscale(image, bottom=bottom, ncolors=ncolors, $
          ...
          square_root=keyword_set(square_root))
    1 : begin
